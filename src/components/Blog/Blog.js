      ...
      return (
            <div className='grid grid-cols-1 lg:grid-cols-2 m-4 p-2'>
                  ...
                  <div>
                        ...
                        <p>In a nutshell, prototypal inheritance is when an object inherits from another object. This differs from classical inheritance, in which a class inherits from another class.
                        ...
                        Prototypal inheritance first appeared in Self and has since appeared in many other languages, but these days most people think of JavaScript when they think of prototypal inheritance.</p>
                  </div>
                  <div>
                        <h1>Why you do not set the state directly in React? </h1>
                        <p>Because of the following reasons, one should never update the state directly:

                        If we alter it directly, executing setState() thereafter may just overwrite your changes.
                        This.state is not changed instantly when we directly update the state. Instead, it generates a pending state transition, which will only yield the current value if accessed after using this function.

                        we'll lose control of the state in all of our components.</p>
                  </div>
                  <div>
                        <h1>How will you implement a search to find products by name?</h1>
                        <p>It can be done in many and many ways. But I think,we should load the data from the database and loaded in state. Then we will may filter byt id or something is match and also using query for this.</p>
                  </div>
                  <div>
                        <h1>What is a unit test? Why should write unit tests?</h1>
                        <p>Unit testing is a process in which smallest part of the software called unit are uniquely individually performing its operation are verified. 
